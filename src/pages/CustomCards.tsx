...
const CustomCards = () => {
  ...
  const fetchCards = async () => {
    const { data } = await (supabase as any).from("custom_cards").select("*").eq("user_id", user!.id).order("created_at", { ascending: false });
    setCards(data || []);
    setLoading(false);
  };
  ...
  if (!user) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-6 px-4">
        ...
        <h1 className="text-4xl font-black text-foreground">Custom Cards</h1>
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-6">
        ...
        <motion.div className="bg-secondary rounded-lg p-4 mb-6 space-y-3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${
                cardType === "white" ? "bg-foreground text-background" : "bg-muted text-muted-foreground"
              }`}
            >
              ⬜ White Card
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${
                cardType === "black" ? "bg-foreground text-background" : "bg-muted text-muted-foreground"
              }`}
            >
              ⬛ Black Card
            </button>
          ...
          <Input
            placeholder={cardType === "black" ? 'Enter prompt (use _ for blanks)' : "Enter answer card text"}
            value={text}
          ...
          <Button onClick={addCard} disabled={!text.trim() || adding} className="bg-accent text-accent-foreground hover:bg-exotic-gold-dim font-bold w-full">
        ...
        <p className="text-xs text-muted-foreground mb-4">
          {blackCards.length} black cards · {whiteCards.length} white cards
          {(blackCards.length > 0 || whiteCards.length > 0) && (
            <span className="text-accent ml-2">Select "My Cards" pack in game to use them!</span>
          )}
        </p>
        ...
        {loading ? (
          ...
          <p className="text-center text-muted-foreground py-10">No custom cards yet. Create some above!</p>
        ) : (
